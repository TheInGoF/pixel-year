...
▶️ **[オンラインで試す](https://theingof.github.io/pixel-year/)** — ブラウザで動作、インストール不要。

🖥️ **アプリのUI** は27言語対応：🇪🇺 EUの全言語 · 🇳🇴 · 🇯🇵

> **Idea 100% human · Code 95% LLM**

<p align="center"><img src="images/gif/pixel-year-demo.gif" alt="Pixel Year — アニメーションデモ" width="384"></p>
...
- **カスタマイズ可能：** マスのサイズ、色（日曜 / 祝日 / 休暇）、すべての線の太さを
  ライブプレビュー付きで調整できます。
- **レイアウト：** ピクセルグリッド（縦／横）と月マトリクス（3×4／4×3、本物の週ミニカレンダー）。
- **週の始まり** 月曜または日曜（既定は国の慣習）；日曜は赤。
- **年を非表示**にするオプション。
- 実寸 5 mm のマス；横レイアウトは A4 に積み重ね、単一の月マトリクスは A5／A6 に印刷。
...
- **ピクセルグリッド** — おなじみの「Year in Pixels」：列＝月、行＝日（1〜31）。**縦**または**横**（転置：列＝日、行＝月。印刷時はA4縦に積み重ね）。
- **月マトリクス（3×4 または 4×3）** — 12 か月の本物の週ミニカレンダー。印刷向けの年間一覧です。1 年分は収まる最小サイズで印刷（**A5**、セルが小さければ **A6** も）；2 年分は A4 一枚に収まります。

<p align="center">
  <img src="images/preview-matrix-ja.png" alt="月マトリクス レイアウト（3×4）" width="300">
  <img src="images/preview-landscape-ja.png" alt="横レイアウト" width="420">
</p>
